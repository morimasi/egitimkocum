...
// FIX: Removed explicit type to allow for correct type inference which resolves middleware type errors.
const app = express();
app.use(cors());
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        const { rows } = await sql`INSERT INTO users (id, name, email, password, role, "profilePicture") VALUES (${id}, ${name}, ${email}, ${password}, ${role}, ${profilePicture}) RETURNING *;`;
...
const createCrudRoutes = (tableName: string) => {
    ...
    router.delete('/', async (req, res) => {
        ...
        try {
            // FIX: The original code used sql template tag incorrectly for dynamic table names and IN clause.
            // Switched to sql.query to be consistent with POST and PUT methods in this factory.
            // Used `id = ANY($1)` which is the correct way to handle array parameters for IN clauses with node-postgres.
...
app.use('/api/calendarEvents', createCrudRoutes('"calendarEvents"')); // Quoted for case sensitivity
...
app.post('/api/conversations/findOrCreate', async (req, res) => {
    ...
    try {
        ...
        const newConversation = {
            id: `conv-${userId1}-${userId2}`,
            participantIds: [userId1, userId2],
            isGroup: false,
        };
        const { rows: newRows } = await sql`INSERT INTO conversations (id, "participantIds", "isGroup") VALUES (${newConversation.id}, ${newConversation.participantIds}, ${newConversation.isGroup}) RETURNING *;`;
...
app.post('/api/gemini/chat', async (req, res) => {
    try {
        ...
        // FIX: The `chat.sendMessage` API expects a string or Part[], not an object with a `message` property.
        // The guideline was likely outdated. This change aligns with the current SDK.
...
// This is a placeholder for a proper seeding mechanism.
app.post('/api/seed', async (_req, res) => {
     try {
        res.status(501).json({ message: "Seed functionality is not fully implemented in this version."});
    } catch (error: any) {
